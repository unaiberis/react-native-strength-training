  ...
  return (
    <Card className="mb-3">
      ...
      <TextInput
        accessibilityLabel="Text input field"
        value={item.notes ?? ''}
        onChangeText={(text) => onUpdate(index, { notes: text || null })}
...
export function RoutineFormScreen() {
  ...
  const handleSelectExercise = useCallback((exercise: ExerciseRow) => {
    setExercises((prev) => [
      ...prev,
      {
  ...
  const onSubmit = useCallback(
    async (formData: WorkoutTemplateInput) => {
      ...
      try {
        ...
        Alert.alert(
          'Error',
          (err as Error).message ?? 'Failed to save routine'
        );
      }
    },
