...
function DayCell({
  ...
  const cellClasses = useMemo(() => {
    ...
    if (isSelected) return `${base} bg-[#2C2C2E]`;
    if (day.isToday) return `${base} bg-[#222225]`;
    return base;
  }, [day.isCurrentMonth, day.isToday, isSelected]);

  const textClasses = useMemo(() => {
    if (day.isToday) return "text-[#F4F4F2] text-sm font-bold";
    return "text-[#F4F4F2] text-sm";
  }, [day.isToday]);
      ...
      {hasWorkouts && (
        <View className="flex-row gap-0.5 mt-0.5">
          {Array.from({ length: Math.min(day.workoutCount, 3) }).map((_, i) => (
            <View
              key={i}
              className="w-1 h-1 rounded-full bg-[#B9B9B6]"
            />
          ))}
        </View>
      )}
...
function MonthHeader({
  ...
  return (
    <View className="flex-row items-center justify-between mb-4">
      ...
      >
        <Text className="text-[#A4A4A8] text-lg">‹</Text>
      </TouchableOpacity>

      <Text className="text-[#F4F4F2] text-base font-bold">
      ...
      <View className="flex-row mb-2">
        {DAY_HEADERS.map((header) => (
          <View key={header} className="flex-1 items-center">
            <Text className="text-[#707074] text-xs font-semibold">
              {header}
            </Text>
          </View>
        ))}
