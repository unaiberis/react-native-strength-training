...
/**
 * Get the day of week for the first day of the month (0=Sun, 6=Sat).
 */
...
export function buildCalendarMonth(
  ...
  const remaining = 42 - days.length; // 6 rows × 7 cols = 42 cells
...
/**
 * Fetch workout sessions for a given month from PocketBase.
 */
async function fetchMonthSessions(
  ...
  for (const s of rows) {
    const dateKey = s.started_at.slice(0, 10); // YYYY-MM-DD
...
export async function fetchSessionsForDate(
  ...
  const endDate = dateStr.slice(0, 8) + String(Number(dateStr.slice(8)) + 1).padStart(2, "0");
  ...
  return Promise.all(
    rows.map(async (s) => {
      ...
      return {
        ...
        durationMinutes: null, // populated via session detail if needed
 ...
 *
 * Fetches completed workout sessions for the given month/year and
 * builds a calendar grid with dots on workout days.
 *
 * Follows the same pattern as useHistory for TanStack Query usage.
 */
