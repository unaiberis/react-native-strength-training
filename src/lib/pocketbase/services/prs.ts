...
function computePR(sets: ExerciseSetRow[]): {
  ...
  // Max reps
  const maxReps =
    workingSets.length > 0 ? Math.max(...workingSets.map((s) => s.reps)) : null;
...
export async function checkIsPR(
  ...
  try {
    ...
    if (
      reps === 1 &&
      (current.oneRepMax === null || weightKg > current.oneRepMax)
    ) {
      return {
        isPR: true,
    ...
    if (
      current.bestVolumeSet === null ||
      proposedVolume > current.bestVolumeSet
    ) {
      return {
        isPR: true,
