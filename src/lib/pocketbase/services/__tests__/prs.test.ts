...
describe("PocketBase PRs service", () => {
  ...
  it("getPRHistory returns progression of best e1RM per session over time", async () => {
    const mockSets = [
      makeSet({ workout_session_id: "sess-1", weight_kg: 80, reps: 10, logged_at: "2026-01-01T10:00:00Z" }),
      makeSet({ id: "set-2", workout_session_id: "sess-2", weight_kg: 90, reps: 6, logged_at: "2026-01-08T10:00:00Z" }),
      makeSet({ id: "set-3", workout_session_id: "sess-2", weight_kg: 100, reps: 3, logged_at: "2026-01-08T10:00:00Z" }),
      makeSet({ id: "set-4", workout_session_id: "sess-3", weight_kg: 95, reps: 5, logged_at: "2026-01-15T10:00:00Z" }),
    ];
  ...
  it("getPRHistory returns progression of one_rep_max over time", async () => {
    const mockSets = [
      makeSet({ workout_session_id: "sess-1", weight_kg: 130, reps: 1, logged_at: "2026-01-01T10:00:00Z" }),
      makeSet({ id: "set-2", workout_session_id: "sess-2", weight_kg: 140, reps: 1, logged_at: "2026-01-08T10:00:00Z" }),
    ];
  ...
  it("getPRHistory throws on PocketBase error", async () => {
    ...
    await expect(getPRHistory("ex-1", "one_rep_max")).rejects.toThrow("PB error");
  });
