...
describe('PocketBase templates service', () => {
  ...
  it('createTemplate creates multiple exercises', async () => {
    ...
    mockCreate.mockResolvedValueOnce(
      makeTemplateExercise({ id: 'te-1', exercise_id: 'ex-1', sort_order: 0 })
    );
    mockCreate.mockResolvedValueOnce(
      makeTemplateExercise({ id: 'te-2', exercise_id: 'ex-2', sort_order: 1 })
    );
  ...
  it('createTemplate throws on template creation failure', async () => {
    ...
    await expect(createTemplate('user-1', validInput)).rejects.toThrow(
  ...
  it('reorderTemplateExercises throws on update failure', async () => {
    ...
    await expect(reorderTemplateExercises('tmpl-1', ['te-1'])).rejects.toThrow(
      'Reorder failed'
    );
  });
});
