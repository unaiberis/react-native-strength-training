...
export class IdMapping {
   ...
   */
  async lookup(localId: string, collection: string): Promise<string | null> {
   ...
   */
  async patchPendingQueue(localId: string, serverId: string): Promise<void> {
    ...
    for (const row of rows) {
      ...
      await this.db.runAsync('UPDATE change_queue SET data = ? WHERE id = ?', [
        patched,
        row.id,
      ]);
    }
