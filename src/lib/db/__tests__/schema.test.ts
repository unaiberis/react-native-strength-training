...
describe('schema migrations', () => {
  ...
  describe('runMigrations', () => {
    ...
    it('creates exercises table with expected columns', async () => {
      ...
      const execCalls = execAsyncMock.mock.calls.map(([sql]: [string]) => sql);
      ...
      expect(createExercises).toContain(
    ...
    it('creates change_queue with autoincrement id', async () => {
      ...
      expect(createQueue).toContain(
        "CHECK(status IN ('pending','in_flight','dead_letter','auth_error'))"
      );
    });
    ...
    it('creates workout_sessions with status CHECK constraint', async () => {
      ...
      expect(createSessions).toContain(
