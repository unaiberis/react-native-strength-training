...
describe("OfflineFeedbackService", () => {
  ...
  describe("submitFeedback", () => {
    ...
    it("handles null coachId and null notes", async () => {
      ...
      // Verify SQLite INSERT includes null values
      const [, params] = db.runAsync.mock.calls[0];
      const paramArray = params as unknown[];
      // Find the coach_id value
      const sql = db.runAsync.mock.calls[0][0] as string;
      const columns = sql.match(/\(([^)]+)\)/)?.[1].split(",").map((c: string) => c.trim()) ?? [];
      const coachIdx = columns.indexOf("coach_id");
