...
describe("startSession", () => {
  it("populates store with session data", () => {
    const exercises = [makeExercise("ex-1", "Squat"), makeExercise("ex-2", "Bench")];
    useSessionStore.getState().startSession("session-1", "template-1", exercises);
...
describe("addLoggedSet", () => {
  ...
  it("appends multiple sets to the same exercise", () => {
    ...
    const set1: LoggedSet = { setNumber: 1, weightKg: 100, reps: 8, rpe: 8, rir: null, isWarmup: false, loggedAt: new Date().toISOString() };
    const set2: LoggedSet = { setNumber: 2, weightKg: 100, reps: 8, rpe: 9, rir: null, isWarmup: false, loggedAt: new Date().toISOString() };
...
describe("clearSession", () => {
  it("resets to initial state", () => {
    ...
    useSessionStore.getState().startSession("s1", "t1", [makeExercise("ex-1", "Squat")]);
    useSessionStore.getState().addLoggedSet("ex-1", {
      setNumber: 1, weightKg: 100, reps: 8, rpe: 8, rir: null, isWarmup: false, loggedAt: new Date().toISOString(),
    });
