...
export default function AuthLayout() {
  ...
  const { state, isTeamCoach, isPendingSignupInfo } = useAuthStore();

  /**
   * If the user is already authenticated, redirect based on team coach status.
   * Team coach → (coach)/, Athlete → (tabs)/
  ...
  useEffect(() => {
    if (state === "authenticated" && !isPendingSignupInfo) {
      const destination = isTeamCoach ? "/(coach)" : "/(tabs)";
      const id = setTimeout(() => router.replace(destination), 0);
      return () => clearTimeout(id);
    }
  }, [state, isTeamCoach, isPendingSignupInfo, router]);
